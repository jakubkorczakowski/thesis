...
class GINet(nn.Module):
    ...
    def forward(self, x, target_idx):
        if self.mode == 'train':
            preds = self._train_forward(x, target_idx)
        else:
            preds = self._test_forward(x, target_idx)

        return preds

    def _test_forward(self, x, co_coding):
        [_, _, H, W] = x.size()
        with torch.no_grad():
    ...
    def _train_forward(self, x, co_coding):
        [_, _, H, W] = x.size()
        x1 = self.backbone.conv1(x)
        x2 = self.backbone.conv2(x1)
        x3 = self.backbone.conv3(x2)
        ...
        p4 = self._upsample_add(p5, self.latlayer4(self._fg_att(x4, _pred)))
        p4 = self.enlayer4(p4)
        _pred = self.dslayer4(p4)
        preds.append(
            F.interpolate(_pred,
        ...
        p3 = self._upsample_add(p4, self.latlayer3(self._fg_att(x3, _pred)))
        p3 = self.enlayer3(p3)
        _pred = self.dslayer3(p3)
        preds.append(
            F.interpolate(_pred,
        ...
        p2 = self._upsample_add(p3, self.latlayer2(self._fg_att(x2, _pred)))
        p2 = self.enlayer2(p2)
        _pred = self.dslayer2(p2)
        preds.append(
            F.interpolate(_pred,
        ...
        p1 = self._upsample_add(p2, self.latlayer1(self._fg_att(x1, _pred)))
        p1 = self.enlayer1(p1)
        _pred = self.dslayer1(p1)
        preds.append(
            F.interpolate(_pred,
